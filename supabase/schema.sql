...
alter table profiles add column if not exists nome_negocio text;
...
create unique index if not exists profiles_telefone_unique_idx on profiles(telefone) where telefone is not null and telefone <> '';
